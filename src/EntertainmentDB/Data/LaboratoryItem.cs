...
{
    /// <summary>
    /// Provides a laboratory item.
    /// </summary>
    public class LaboratoryItem : EntryItem
    {
        // --- Properties ---

        /// <summary>
        /// The laboratory of the laboratory item.
        /// </summary>
        ...
        /// <summary>
        /// Initializes a laboratory item with the given reader and empty id string and table names.
        /// </summary>
        /// <param name="reader">The database reader to be used to read the laboratory item information from the database.</param>
        public LaboratoryItem(DBReader reader) : this(reader, "", "", "")
        {
        }
        ...
        /// <param name="baseTableName">The base table name of the laboratory item.</param>
        ...
        public LaboratoryItem(DBReader reader, string id, string baseTableName, string targetTableName) : base(reader, id, baseTableName, targetTableName)
        {
            Logger.Trace($"LaboratoryItem(): LaboratoryItem with ID = '{id}' created");
        }

        // --- Methods ---

        /// <summary>
        /// Retrieves the basic information of the laboratory item from the database.
        /// </summary>
        /// <param name="retrieveBasicInfoOnly">true if only the basic info is to be retrieved; false if also additional data is to be retrieved.</param>
        /// <returns>1 if data record was retrieved; 0 if no data record matched the id.</returns>
        protected override int RetrieveBasicInformation(bool retrieveBasicInfoOnly)
        {
            Logger.Trace($"LaboratoryItem.RetrieveBasicInformation()");

            Reader.Query = $"SELECT ID, LaboratoryID, Details, StatusID, LastUpdated " +
            ...
            if (noOfDataRecords == 1)
            {
                ...
                ID = row["ID"].ToString();
                if (!String.IsNullOrEmpty(row["LaboratoryID"].ToString()))
                {
                    Logger.Debug($"AwardItem.StatusID is not null -> retrieve");

                    Laboratory = new Laboratory(Reader.New());
                    Laboratory.ID = row["LaboratoryID"].ToString();
                    Laboratory.Retrieve(retrieveBasicInfoOnly);
                }
        ...
        /// <summary>
        /// Retrieves a list of laboratory items from the database.
        /// </summary>
        /// <param name="reader">The reader to be used to retrieve the data records.</param>
        /// <param name="baseTableName">The base table name of the laboratory item.</param>
        /// <param name="baseTableID">The base table id of the laboratory item.</param>
        /// <param name="targetTableName">The target table name of the laboratory item.</param>
        /// <param name="order">The order in which the data records are to be sorted.</param>
        /// <returns>The list of laboratory items.</returns>
        /// <exception cref="ArgumentNullException">Thrown when any of the given parameters is null.</exception>
        ...
        {
            Logger.Trace($"LaboratoryItem.RetrieveList()");
            ...
            if (noOfDataRecords > 0)
            {
                ...
                {
                    LaboratoryItem item = new LaboratoryItem(reader.New());
