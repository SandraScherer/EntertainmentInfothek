...
namespace EntertainmentDB.Data.Tests
{
    ...
    public class ColorItemTests
    {
        ...
        [TestMethod()]
        public void ColorItemTest_withID()
        {
            // Arrange
            ColorItem item = new ColorItem("_xx1", "Color");
            ...
            Assert.IsNotNull(item.Reader);
            Assert.AreEqual("", item.BaseTableName);
            Assert.AreEqual("Color", item.TargetTableName);
        ...
        public void RetrieveBasicInformationTest_withValidID_BasicInfoOnly(string value)
        {
            ...
            Assert.AreEqual($"{value} Color Details X1", item.Details);
            Assert.AreEqual("_xxx", item.Status.ID);
            Assert.AreEqual($"{value} Color Last Updated X1", item.LastUpdated);
        }
        ...
        {
            // Arrange
            ColorItem item = new ColorItem("_aa1", "Color");
            item.BaseTableName = value;
        ...
        public void RetrieveListTest_withValidData(string value)
        {
            ...
            // Act
            List<ColorItem> list = Data.ColorItem.RetrieveList(reader, value, "_xxx", "Color");
            ...
            Assert.AreEqual($"{value} Color Details X1", list[0].Details);
            Assert.AreEqual("_xxx", list[0].Status.ID);
            Assert.AreEqual($"{value} Color Last Updated X1", list[0].LastUpdated);

            Assert.AreEqual("_xx2", list[1].ID);
            Assert.AreEqual("_yyy", list[1].Color.ID);
            Assert.AreEqual($"{value} Color Details X2", list[1].Details);
            Assert.AreEqual("_xxx", list[1].Status.ID);
            Assert.AreEqual($"{value} Color Last Updated X2", list[1].LastUpdated);

            Assert.AreEqual("_xx3", list[2].ID);
            Assert.AreEqual("_zzz", list[2].Color.ID);
            Assert.AreEqual($"{value} Color Details X3", list[2].Details);
            Assert.AreEqual("_xxx", list[2].Status.ID);
            Assert.AreEqual($"{value} Color Last Updated X3", list[2].LastUpdated);
        }
    }
}
