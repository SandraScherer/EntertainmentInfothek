...
{
    [TestClass()]
    public class CinematographicProcessItemTests
    {
        [TestMethod()]
        ...
        [TestMethod()]
        public void CinematographicProcessItemTest_withID()
        {
            // Arrange
            CinematographicProcessItem item = new CinematographicProcessItem("_xx1", "CinematographicProcess");
            ...
            Assert.IsNotNull(item.Reader);
            Assert.AreEqual("", item.BaseTableName);
            Assert.AreEqual("CinematographicProcess", item.TargetTableName);
        ...
        public void RetrieveBasicInformationTest_withValidID_BasicInfoOnly(string value)
        {
            ...
            Assert.AreEqual($"{value} Cinematographic Process Details X1", item.Details);
            Assert.AreEqual("_xxx", item.Status.ID);
            Assert.AreEqual($"{value} Cinematographic Process Last Updated X1", item.LastUpdated);
        }
        ...
        {
            // Arrange
            CinematographicProcessItem item = new CinematographicProcessItem("_aa1", "CinematographicProcess");
            item.BaseTableName = value;
        ...
        public void RetrieveListTest_withValidData(string value)
        {
            ...
            // Act
            List<CinematographicProcessItem> list = Data.CinematographicProcessItem.RetrieveList(reader, value, "_xxx", "CinematographicProcess");
            ...
            Assert.AreEqual($"{value} Cinematographic Process Details X1", list[0].Details);
            Assert.AreEqual("_xxx", list[0].Status.ID);
            Assert.AreEqual($"{value} Cinematographic Process Last Updated X1", list[0].LastUpdated);

            Assert.AreEqual("_xx2", list[1].ID);
            Assert.AreEqual("_yyy", list[1].CinematographicProcess.ID);
            Assert.AreEqual($"{value} Cinematographic Process Details X2", list[1].Details);
            Assert.AreEqual("_xxx", list[1].Status.ID);
            Assert.AreEqual($"{value} Cinematographic Process Last Updated X2", list[1].LastUpdated);

            Assert.AreEqual("_xx3", list[2].ID);
            Assert.AreEqual("_zzz", list[2].CinematographicProcess.ID);
            Assert.AreEqual($"{value} Cinematographic Process Details X3", list[2].Details);
            Assert.AreEqual("_xxx", list[2].Status.ID);
            Assert.AreEqual($"{value} Cinematographic Process Last Updated X3", list[2].LastUpdated);
        }
    }
}
