...
{
    [TestClass()]
    public class LocationItemTests_WithDB
    {
        const string VALID_ID = "_xx1";
        const string INVALID_ID = "_aa1";

        [TestMethod()]
        public void LocationItemTest()
        {
            // Arrange
        ...
        [DataRow(VALID_ID)]
        [DataRow(INVALID_ID)]
        public void LocationItemTest_withID(string id)
        {
            // Arrange
            DBReader reader = new SQLiteReader();
            LocationItem item = new LocationItem(reader, id, "BaseTable", "TargetTable");
        ...
        public void RetrieveBasicInformationTest_withValidID(string baseTableName, bool basicInfoOnly)
        {
            // Arrange
            DBReader reader = new SQLiteReader();
            LocationItem item = new LocationItem(reader, VALID_ID, baseTableName, "FilmingLocation");
            ...
            Assert.AreEqual($"{baseTableName} FilmingLocation Details X1", item.Details);
            Assert.AreEqual("_xxx", item.Status.ID);
            Assert.AreEqual($"{baseTableName} FilmingLocation LastUpdated X1", item.LastUpdated);
        }
        ...
        public void RetrieveBasicInformationTest_withInvalidID(string baseTableName, bool basicInfoOnly)
        {
            // Arrange
            DBReader reader = new SQLiteReader();
            LocationItem item = new LocationItem(reader, INVALID_ID, baseTableName, "FilmingLocation");
        ...
        public void RetrieveAdditionalInformationTest(string baseTableName, string id)
        {
            // Arrange
            DBReader reader = new SQLiteReader();
            LocationItem item = new LocationItem(reader, id, baseTableName, "FilmingLocation");
        ...
        public void RetrieveListTest_withValidData(string baseTableName)
        {
            ...
            // Act
            List<LocationItem> list = Data.LocationItem.RetrieveList(reader, baseTableName, "_xxx", "FilmingLocation");
            ...
            Assert.AreEqual($"{baseTableName} FilmingLocation Details X1", list[0].Details);
            Assert.AreEqual("_xxx", list[0].Status.ID);
            Assert.AreEqual($"{baseTableName} FilmingLocation LastUpdated X1", list[0].LastUpdated);

            Assert.AreEqual("_xx2", list[1].ID);
            Assert.AreEqual("_yyy", list[1].Location.ID);
            Assert.AreEqual($"{baseTableName} FilmingLocation Details X2", list[1].Details);
            Assert.AreEqual("_xxx", list[1].Status.ID);
            Assert.AreEqual($"{baseTableName} FilmingLocation LastUpdated X2", list[1].LastUpdated);

            Assert.AreEqual("_xx3", list[2].ID);
            Assert.AreEqual("_zzz", list[2].Location.ID);
            Assert.AreEqual($"{baseTableName} FilmingLocation Details X3", list[2].Details);
