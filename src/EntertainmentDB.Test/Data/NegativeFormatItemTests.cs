...
namespace EntertainmentDB.Data.Tests
{
    ...
    public class NegativeFormatItemTests
    {
        ...
        {
            // Arrange
            NegativeFormatItem item = new NegativeFormatItem("_xx1");
        ...
        public void RetrieveBasicInformationTest_withValidID(string value)
        {
            ...
            Assert.AreEqual($"{value} Negative Format Details X1", item.Details);
            Assert.AreEqual("_xxx", item.Status.ID);
            Assert.AreEqual($"{value} Negative Format Last Updated X1", item.LastUpdated);
        }
        ...
        {
            // Arrange
            NegativeFormatItem item = new NegativeFormatItem("_aa1");
            item.BaseTableName = value;
        ...
        public void RetrieveListTest_withValidData(string value)
        {
            ...
            // Act
            List<NegativeFormatItem> list = Data.NegativeFormatItem.RetrieveList(reader, value, "_xxx");
            ...
            Assert.AreEqual($"{value} Negative Format Details X1", list[0].Details);
            Assert.AreEqual("_xxx", list[0].Status.ID);
            Assert.AreEqual($"{value} Negative Format Last Updated X1", list[0].LastUpdated);

            Assert.AreEqual("_xx2", list[1].ID);
            Assert.AreEqual("_yyy", list[1].FilmFormat.ID);
            Assert.AreEqual($"{value} Negative Format Details X2", list[1].Details);
            Assert.AreEqual("_xxx", list[1].Status.ID);
            Assert.AreEqual($"{value} Negative Format Last Updated X2", list[1].LastUpdated);

            Assert.AreEqual("_xx3", list[2].ID);
            Assert.AreEqual("_zzz", list[2].FilmFormat.ID);
            Assert.AreEqual($"{value} Negative Format Details X3", list[2].Details);
            Assert.AreEqual("_xxx", list[2].Status.ID);
            Assert.AreEqual($"{value} Negative Format Last Updated X3", list[2].LastUpdated);
        }
    }
}
