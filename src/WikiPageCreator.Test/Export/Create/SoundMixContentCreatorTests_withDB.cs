...
{
    [TestClass()]
    public class SoundMixContentCreatorTests_withDB
    {
        ...
        public void SoundMixContentCreatorTest(string id, string targetLanguageCode)
        {
            // Arrange
            ...
            // Act
            SoundMixContentCreator creator = new SoundMixContentCreator(list, formatter, targetLanguageCode);

            // Assert
            Assert.IsNotNull(creator);
            Assert.AreEqual(list, creator.SoundMixes);
            Assert.AreEqual(formatter, creator.Formatter);
            Assert.AreEqual(targetLanguageCode, creator.TargetLanguageCode);
            Assert.AreEqual("SoundMix", creator.Headings["en"]);
            Assert.AreEqual("Tonmischung", creator.Headings["de"]);
        }
        ...
        [DataRow("de")]
        [DataRow("zz")]
        public void SoundMixContentCreatorTest_withSoundMixesNull(string targetLanguageCode)
        {
            // Arrange
            Formatter formatter = new DokuWikiFormatter();
            SoundMixContentCreator creator;

            // Act, Assert
            Assert.ThrowsException<NullReferenceException>(() => creator = new SoundMixContentCreator(null, formatter, targetLanguageCode));
        }
        ...
        [DataRow(INVALID_ID, "de")]
        [DataRow(INVALID_ID, "zz")]
        public void SoundMixContentCreatorTest_withFormatterNull(string id, string targetLanguageCode)
        {
            // Arrange
            DBReader reader = new SQLiteReader();
            SoundMix entry = new SoundMix(reader, id);
            ...
            // Act, Assert
            Assert.ThrowsException<ArgumentNullException>(() => creator = new SoundMixContentCreator(list, null, targetLanguageCode));
        }

        [DataTestMethod()]
        [DataRow(VALID_ID)]
        [DataRow(INVALID_ID)]
        public void SoundMixContentCreatorTest_withTargetLanguageCodeNull(string id)
        {
            // Arrange
            DBReader reader = new SQLiteReader();
            SoundMix entry = new SoundMix(reader, id);
            ...
            // Act, Assert
            Assert.ThrowsException<ArgumentNullException>(() => creator = new SoundMixContentCreator(list, formatter, null));
        }

        [DataTestMethod()]
        [DataRow(VALID_ID)]
        [DataRow(INVALID_ID)]
        public void SoundMixContentCreatorTest_withTargetLanguageCodeEmptyString(string id)
        {
            // Arrange
            DBReader reader = new SQLiteReader();
            SoundMix entry = new SoundMix(reader, id);
            ...
            // Act, Assert
            Assert.ThrowsException<ArgumentNullException>(() => creator = new SoundMixContentCreator(list, formatter, ""));
        }
        ...
        public void CreateInfoBoxContentTest_withValidID(string targetLanguageCode)
        {
            ...
            if (targetLanguageCode.Equals("en"))
            {
                testContent.Add(formatter.AsTableRow(
                    new string[] { "SoundMix",
                                   formatter.AsInternalLink(pathInfo, "SoundMix EnglishTitle X", "SoundMix EnglishTitle X") }));
            }
            else
            {
                testContent.Add(formatter.AsTableRow(
                    new string[] { "Tonmischung",
                                   formatter.AsInternalLink(pathInfo, "SoundMix EnglishTitle X", "SoundMix GermanTitle X") }));
            }
